...
type UnknownKeyword struct{ Keyword, File string; Line int }
// UnknownKeywordError is returned when the config uses a keyword ssh_config(5)
// does not define. It names every one it found, with file and line.
type UnknownKeywordError struct{ Unknown []UnknownKeyword }
// IgnoredKeywords lists the keywords cfg sets for alias that this package
// defines but does not act on — what `doctor` reports for a host.
...
// Every keyword in the file, and in the files it Includes, is first checked
// against the set ssh_config(5) defines (keywords.go, regenerated by
// keywords_gen.py); one that is not defined is an UnknownKeywordError unless
// an IgnoreUnknown pattern covers it. Added for issue #21.
