...
- `TrustPromptMarkers`, `HasTrustPrompt(text) (string, bool)` and `TrustPromptWaiting` (ruling R-P3-TRUST-1) — Claude Code's first-run trust dialog ("Quick safety check" / "Yes, I trust this folder"), deliberately NOT a failure marker: a Claude at that dialog resumed fine, it simply writes no registry entry until answered. `TrustPromptWaiting` opens the ConfirmClaude error for the untrusted case, and `internal/cli`'s `startFailureHint` matches on it so the `/login` advice never fires for it
...
- `FAKECLAUDE_TRUST_PROMPT=1` (ruling R-P3-TRUST-1, a deliberate spec §12 addition): an interactive run prints Claude Code's first-run trust dialog BEFORE writing any registry entry and waits for it to be answered — Down (`\x1b[B`/`\x1bOB`) then Enter/CR is "Yes, I trust this folder"; a bare Enter is "No, exit" (exit 1). It prompts unconditionally, without consulting the global config, so a test can exercise the auto-accept path even after the trust has been merged. Verified against real Claude Code 2.1.259, which shows the same dialog and likewise registers nothing until it is answered
