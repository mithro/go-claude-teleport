...
- `CaptureScreen(ctx, t, paneID) ([]byte, error)` — `capture-pane -epJ` **without** `-S -`: the pane's VISIBLE screen only (ruling R-P3-TRUST-1). "Is Claude showing its trust dialog right now" is a question about the current screen; the same words in the scrollback record a dialog already answered, and answering that one would type into a live Claude
...
- `Local.SessionExtras` fills `InstallExtras.TrustCwd`/`SourceTrusted`: the session cwd's own project entry first, then the main repository's when the cwd is a linked git worktree (`gitx.Inspect`), path-mapped into the destination's paths and logged either way (ruling R-P3-TRUST-1 item 1)
...
- `TrustPromptMarkers`, `HasTrustPrompt(text) (string, bool)`, `TrustAnswerSelected(text) bool` and `TrustPromptWaiting` (ruling R-P3-TRUST-1) — Claude Code's first-run trust dialog ("Quick safety check" / "Yes, I trust this folder"), deliberately NOT a failure marker: a Claude at that dialog resumed fine, it simply writes no registry entry until answered. A single phrase is not enough to call it a dialog (a resumed conversation may QUOTE it): it takes a rendered selection line (`❯ No, exit` / `❯ Yes, I trust this folder`, which only the dialog draws) or both phrases together. `TrustAnswerSelected` is the same check for "is Yes the selected choice", which the confirm step verifies after Down and before Enter — an unmoved selection means Enter would answer "No, exit". `TrustPromptWaiting` opens the ConfirmClaude error for the untrusted case, and `internal/cli`'s `startFailureHint` matches on it so the `/login` advice never fires for it
- `Endpoint.ConfirmClaude(ctx, ref, id, timeout, trusted bool)` and `ConfirmClaudeArgs.Trusted bool` (`json:"trusted,omitempty"`) — the source's carried trust. Trusted: the dialog is answered once per call (Down, then Enter, into the job's own sigil-guarded pane) and the poll keeps waiting for the registry; untrusted: nothing is typed and the error names the host, the pane and the `continue` command. The scan runs only while there is no live registry entry, and only over `tmuxx.CaptureScreen`
- `Local.runStream`'s `tar send` subtracts `planView.DestOwnedIDs` (`json:"dest_owned_ids"`) from `transfer.Need` — the source never even offers a file the destination's live Claude owns (ruling R-P3-TRUST-1 item 3)
...
- `InstallExtras.TrustCwd string` (`json:"trust_cwd,omitempty"`) / `InstallExtras.SourceTrusted bool` (`json:"source_trusted,omitempty"`) and `InstallReport.TrustGranted bool` (ruling R-P3-TRUST-1 item 1) — the source's answer to Claude Code's first-run trust dialog and the DESTINATION path whose project entry grants it (`ProjectCwd`, or the mapped MAIN repository path when the cwd is a linked git worktree, which is where real Claude Code 2.1.259 keys it). `Install` grants it via `session.GrantProjectTrust` before the start step, idempotently, and refuses a `TrustCwd` that is not absolute and clean or that is the destination's own Home/ConfigDir/DataDir/`/`
...
- `Plan.DestOwnedIDs []int` (`json:"dest_owned_ids,omitempty"`, ruling R-P3-TRUST-1 item 3) — the manifest ids the DESTINATION's own live Claude owns (its `CatSession` files, once the destination registry shows the session alive in this job's own `DestRef`). It lives on the plan, and so travels to both hosts' journals, because the SOURCE's tar stream reads it (`remote.planView`) and stops offering those entries; capture then reports done, transfer subtracts them from `Pending`, and install neither places nor demands them. `Plan.sourceTrusted()` is the nil-safe accessor the start step passes to `ConfirmClaude`
...
- `TrustAccepted(e ProjectEntry) bool` and `GrantProjectTrust(globalJSON, cwd string) (granted bool, err error)` (ruling R-P3-TRUST-1) — read/set `projects.<cwd>.hasTrustDialogAccepted`, the field real Claude Code 2.1.259 records its first-run trust dialog in (keyed by the MAIN repository path for a linked-worktree session). The grant creates the entry when absent, otherwise sets only that field, and is idempotent; it shares one backup + temp-file + rename writer with `AddProjectEntry`
...
**test/fakeclaude**
- `FAKECLAUDE_TMUX`, `FAKECLAUDE_FAIL=not-logged-in` and `FAKECLAUDE_RUN_CHILD` are Plan 01 Task 19's env contract, used as defined there
- `FAKECLAUDE_TRUST_PROMPT=1` (ruling R-P3-TRUST-1, a deliberate spec §12 addition): an interactive run prints Claude Code's first-run trust dialog BEFORE writing any registry entry and waits for it to be answered — Down (`\x1b[B`/`\x1bOB`) then Enter/CR is "Yes, I trust this folder"; a bare Enter is "No, exit" (exit 1). Like the real thing it REDRAWS with the selection moved as soon as Down arrives, which needs the tty in raw mode for the question (via `stty`, restored immediately afterwards — a raw tty would turn the later `/exit` into a CR the scanner never sees; a non-tty stdin needs neither). It prompts unconditionally, without consulting the global config, so a test can exercise the auto-accept path even after the trust has been merged. Verified against real Claude Code 2.1.259, which shows the same dialog and likewise registers nothing until it is answered
