...
func TestServeHelloAndProtocolMismatch(t *testing.T) {
	...
	rs := roundTrip(t, ep,
		fmt.Sprintf(`{"id":1,"op":"hello","args":{"version":"v0.3","protocol":%d}}`, version.Protocol),
	...
	if rs[1].OK || rs[1].Error == nil || rs[1].Error.Code != "usage" || !strings.Contains(rs[1].Error.Message, "99") || !strings.Contains(rs[1].Error.Message, fmt.Sprint(version.Protocol)) {
...
func TestServeOverNetPipeStopsOnClose(t *testing.T) {
	...
	io.WriteString(b, fmt.Sprintf(`{"id":1,"op":"hello","args":{"protocol":%d}}`, version.Protocol)+"\n")
